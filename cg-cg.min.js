...
function fetchContent( device ){
    ...
    if( terminal_mode === "H" ){
        ...
        var url = "https://tpcg1.tutorialspoint.com/tpcg.php";
        if( langid ==="matplotlib" || langid === "numpy" || langid ==="scipy" || langid === "python3" || langid === "python" || langid === "cpp" || langid === "c99" || langid === "cpp0x" || langid === "cpp11" || langid === "c" || langid == "java" || langid == "java8" || langid == "php"){
            url = "https://tpcg2.tutorialspoint.com/tpcg.php";
        }
...
function generateLink(tabindex) {
   console.log( "Going to generate link....");
   var content = codeeditor.getValue();
   if (content.length > 0 ) {
...
function userLogout(){
    ...
    $.ajax({
		...
		$('#codebox').tabs('select', 0);
                ...
                contentcover.html('<div style="padding-left:10px;"><pre style="margin:5px;">Logged Out Successfully</pre></div>');
            }
            $("#loading").css({ "visibility": "hidden" });
        }
    });
                              ...
                              $.ajax({
                               ...
                               success: function(data) {
                                   if (data.status) {
                                       ...
                                       $("h1.main-title").text( projecttitle + " (" + version + ")");
				       contentcover.html('<div style="padding-left:10px;"><pre style="margin:5px;">Project title changed and saved successfully.</pre></div>');
                                   }
                                   $("#loading").css({ "visibility": "hidden" });
                               }
                              });
...
function openShareLink( linkid ) {
    ...
    $win.window('open');
    $win.window('center');
    $('#dialog').window('refresh', SERVER + "/show_permalink.php?permalink="+linkid);
}
